...
export class DropZone {
  ...
  public move(item: HTMLElement, toIndex: number): void {
    ...
    const sortableIndices = allChildren
      .map((child, index) => child.classList.contains('sortable-item') ? index : -1)
    ...
    if (toIndex >= sortableIndicesWithoutCurrent.length) {
      ...
      if (sortableIndicesWithoutCurrent.length > 0) {
        targetDOMIndex = sortableIndicesWithoutCurrent[sortableIndicesWithoutCurrent.length - 1] + 1
      } else {
