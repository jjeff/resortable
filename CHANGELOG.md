...
- **`resortable/react` shipped with no usable types in 3.0.0.** `package.json` pointed `exports["./react"].types` (and the matching `typesVersions` entry) at `dist/types/react/index.d.ts`, a file 3.0.0 does not contain — the `vite-plugin-dts` major upgrade in that release changed where declarations land, from `dist/types/` to mirroring `src/` at the dist root, and the manifest was never updated. TypeScript consumers got `TS7016: Could not find a declaration file for module 'resortable/react'`, so `useSortable` resolved to `any` and every property read off its events became a lint error in their build. 2.3.0 is unaffected. The declared paths now match what the build produces.
- **Nothing checked that the published package's entry points exist.** Unit and e2e suites run against `src/`, and bundle-size only weighs the bundles, so the artifact users actually install was the one thing no job loaded — which is why the above shipped with CI fully green. `npm run check:exports` now verifies every path named by `main`, `module`, `unpkg`, `types`, `exports` and `typesVersions`, and runs in the `build` job.
- The `outDir: 'dist/types'` option passed to `vite-plugin-dts` in both build configs was silently ignored by the current major. Removed, with a note recording where declarations actually go, so the config stops describing a layout the build does not produce.
...
- **`nativeDrag` makes the native HTML5 drag pipeline reachable** (#165): Resortable attached both pipelines, but only the pointer one could ever run — `onPointerDown` calls `preventDefault()` on every drag-eligible press, and a prevented `pointerdown` stops the browser firing `dragstart`. The HTML5 handlers, and the documented `setData` option that lives inside them, were unreachable in every configuration, so no drag could carry a payload out of the document to another window or application. `nativeDrag: true` hands the gesture to the native pipeline instead. Touch always falls back to the pointer pipeline, since HTML5 drag-and-drop has no mobile support. `nativeDrag` and `forceFallback` are contradictory — `forceFallback` unbinds the listeners `nativeDrag` needs — so setting both keeps the pointer pipeline and warns.
- **The native pipeline reached parity with the pointer one** (#165). Multi-select drag: dragging an already-selected item drags the whole selection in DOM order, and the run moves together on drop instead of the anchor moving and the rest being stranded. Modifier handling now routes through the same predicate both pipelines share, so a configured `duplicateKey` no longer reads as a selection click (`duplicateKey: 'ctrl'` could not start a native drag at all before). `duplicateKey` itself works on drop, signalled through `effectAllowed`/`dropEffect` because a native drag session gives CSS `cursor` no effect. And a multi-item drag gets the stacked ghost — layered shadow, count badge — handed to `setDragImage`, where before the ghost was hidden and the browser drew the single grabbed item.
- **`setDragCursor(cursor)` sets the cursor for the current drag** (#165): a consumer had no way to signal that a drop will copy rather than move. The browser draws the cursor from the pointer-capture target, controlled mode hides that target with `display: none` so it paints no cursor at all, and `syncDuplicate` overwrote any inline cursor on the next pointermove. The override is a document-wide `!important` rule for the length of the gesture, cleared automatically when the drag ends; on a native drag it maps onto `dropEffect`, which supports `copy`, `move`, `link`/`alias` and `none`/`no-drop`/`not-allowed`.

### Bug Fixes

- **Sortable zones no longer swallow drags they did not start** (#165): every zone called `preventDefault` and `stopPropagation` on any `dragover`, `dragenter` and `drop` before checking whether the drag was its own. A file dragged in from the desktop, or a drag from another window, was claimed by the nearest sortable and the consumer's own drop handler never fired. **This affected every consumer of 2.3.0 as shipped, with no `nativeDrag` involved** — the HTML5 listeners are attached in the default configuration, and only these three handlers' `preventDefault` ordering was at fault. The ownership test now runs first, and `drop` is group-aware like its siblings rather than accepting any drag in flight.
- **Cross-zone native drags now land where the pointer is, and clean up after themselves** (#165): three separate faults, all of them only reachable once native drags can run. `onDragOver` and `onDrop` read the placeholder off the manager of the zone being dragged *over*, but the placeholder belongs to the zone the drag *started* in — so it was always null cross-zone, the whole placement block was skipped, and every cross-zone drop landed wherever the pointer first crossed the boundary. A multi-item cross-zone drag inserted only the anchor and stranded the rest in the source list while the end events still reported all of them. And `dragend` fires on the dragged element, which by then lives in the target zone, so the source manager's teardown never ran — leaving its placeholder in its own list, its keydown/keyup listeners on `document`, and its drag state stuck on.
- **Destroying a Sortable mid-drag no longer poisons every later drag** (#165): `detach()` handled an in-flight pointer drag and ignored an in-flight native one, while removing the `dragend` listener that was the only thing left to end it. The `'html5-drag'` entry survived in the shared drag state, so the ownership checks above went on matching it and the next foreign drag over any same-group zone was claimed and run against detached items.
- **A drag into the viewport's top-left corner is no longer read as coordinate-less** (#165): `(0, 0)` is a sentinel for synthetic events — jsdom defaults to it and Firefox under Playwright freezes coordinates there — but a real corner drag hits it too, dropping the grab origin and freezing sorting for any `swapThreshold > 0`. The sentinel is now gated on `isTrusted`.
- **A touchscreen laptop no longer loses native drag for its mouse** (#165): both the `dragstart` bail and the `draggable` attribute keyed off `navigator.maxTouchPoints`, which reports what the device is capable of rather than what started the gesture. Under `nativeDrag` the question is now what actually pressed.

### Documentation

- `forceFallback` was documented as defaulting to `true`; it defaults to `false`. The name also reads backwards — `false` does not mean native drag runs, it means the native listeners are attached and still never fire. Both are now stated.
- `setData` was documented as a supported option without noting that it only runs on the native pipeline and so needs `nativeDrag`.
- `dragAndDropWithAnimation` (e2e helper) claimed `page.dragAndDrop` drives the native HTML5 pipeline on desktop. It does not, on any project — the pointer pipeline suppresses the native drag first.

<!-- /#165 native drag -->

<!-- #150 duplicate affordance -->

### Bug Fixes

- **`duplicateKey` now previews what it will do** (#150): holding the modifier gave no feedback at all — the cursor stayed a plain arrow, and the drag preview showed the original leaving its home slot even though an alt-drop leaves it exactly where it was. Both trace to one detail: the dragged item holds pointer capture, the browser draws the cursor from the capture target rather than from the element under the pointer, and controlled mode hides that item with `display: none` — a capture target with no layout box paints no cursor at all, so no consumer CSS could reach it. `syncDuplicate` now un-hides the originals while the modifier is armed (the preview matches the post-drop state: original stays home, placeholder previews the incoming copy) and sets `cursor: copy` on the dragged item, which is drawable precisely because it is visible again. Both halves flip live on keydown/keyup. The preview un-hide is controlled-mode only; uncontrolled drags never hide the item, so the cursor works there without it. Capture deliberately stays on the dragged item — moving it to `<html>` also fixes the cursor but retargets the trailing `click` away from the item, breaking ctrl/shift-click multi-select.
