...
- **Sortable zones no longer swallow drags they did not start** (#165): every zone called `preventDefault` and `stopPropagation` on any `dragover`, `dragenter` and `drop` before checking whether the drag was its own. A file dragged in from the desktop, or a drag from another window, was claimed by the nearest sortable and the consumer's own drop handler never fired. The ownership test now runs first. This was unobservable while the native pipeline was unreachable; it is exactly the case `nativeDrag` exists for.
