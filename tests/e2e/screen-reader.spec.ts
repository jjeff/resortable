...
test.describe('Screen Reader Support', () => {
  ...
  test('provides live region announcements for drag operations', async ({ page }) => {
  ...
  test('provides descriptive ARIA labels', async ({ page }) => {
    ...
    await expect(container).toHaveAttribute('aria-label', /Sortable list.*arrow keys.*space.*enter/)
  })
  ...
  test('maintains proper ARIA position attributes', async ({ page }) => {
    ...
    for (let i = 0; i < 4; i++) {
      await expect(items.nth(i)).toHaveAttribute('aria-posinset', (i + 1).toString())
    ...
    for (let i = 0; i < 4; i++) {
      await expect(newItems.nth(i)).toHaveAttribute('aria-posinset', (i + 1).toString())
      await expect(newItems.nth(i)).toHaveAttribute('aria-setsize', '4')
    }
  })

  test('provides keyboard instructions in ARIA descriptions', async ({ page }) => {
    const container = page.locator('#basic-list')
  ...
  test('clears announcements after delay to allow re-announcement', async ({ page }) => {
    const announcer = page.locator('[role="status"][aria-live="assertive"]')
    const firstItem = page.locator('#basic-list [data-id="basic-1"]')
  ...
  test('maintains focus management for screen reader users', async ({ page }) => {
    const firstItem = page.locator('#basic-list [data-id="basic-1"]')
    const secondItem = page.locator('#basic-list [data-id="basic-2"]')
