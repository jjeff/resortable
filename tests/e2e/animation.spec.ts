...
test.describe('Animation System', () => {
  ...
  test('should animate item reordering with FLIP technique', async ({ page }) => {
  ...
  test('should animate ghost element appearance and removal', async ({ page }) => {
    // Enable ghost class
    await page.evaluate(() => {
      const list = document.getElementById('simple-list')
      if (list) {
  ...
  test('should handle rapid successive drags with animation cancellation', async ({ page }) => {
    const items = page.locator('.sortable-item')
    const firstItem = items.nth(0)
    const secondItem = items.nth(1)
