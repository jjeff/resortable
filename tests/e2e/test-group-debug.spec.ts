    ...
    document.addEventListener('dragover', (e) => {
      ...
      if (container) {
        ...
        try {
          ...
          if (globalDragState) {
            const canAccept = globalDragState.canAcceptDrop('html5-drag', container.dataset.sortableGroup)
            console.log(`[DRAGOVER] Container ${container.id} can accept: ${canAccept}`)
          }
        } catch (err) {
      ...
      setTimeout(() => {
        try {
          ...
          if (globalDragState) {
            ...
            if (activeDrag) {
              console.log('[DRAGSTART] Active drag group:', activeDrag.groupName)
              console.log('[DRAGSTART] Can accept drop to shared-horizontal:',
                globalDragState.canAcceptDrop('html5-drag', 'shared-horizontal'))
            }
          }
        } catch (err) {
