...
describe('scroll-replay coalescing during autoscroll (#134)', () => {
  ...
  it('notifyProgrammaticScroll refreshes the target with no scroll event', () => {
    // The whole coalescing/flush mechanism above hangs off the DOM `scroll`
    // event. That event is async and coalesced, so a programmatic scroll can
    // still be undelivered when the pointer is released — no event means no
    // pending frame, which means the drop-time flush has nothing to flush and
    // the drop commits a pre-scroll target. Measured on the Linux WebKit CI
    // leg under load: index 5 and 14 where 29 was correct.
    //
    // AutoScrollPlugin therefore calls notifyProgrammaticScroll() at the
    // scroll site. Note this test never dispatches a `scroll` event.
    vi.useFakeTimers()
    const list = makeList('list')
    const sortable = mount(list, { animation: 0 })
    const internals = sortable.dragManager as unknown as {
    ...
    // Stands in for AutoScrollPlugin mutating scrollTop on its rAF loop.
    internals.notifyProgrammaticScroll()
    internals.notifyProgrammaticScroll()

    // Same per-frame coalescing as the event path: still just the seed call.
    expect(replay).toHaveBeenCalledTimes(1)
    vi.advanceTimersByTime(16)
    expect(replay).toHaveBeenCalledTimes(2)
  })

  it('a drop after a programmatic scroll flushes without any scroll event', () => {
    // The case that actually broke CI: scroll, then release before the frame
    // fires, with the `scroll` event never delivered at all.
    vi.useFakeTimers()
    const list = makeList('list')
    const sortable = mount(list, { animation: 0 })
    const internals = sortable.dragManager as unknown as {
    ...
    // Flushed synchronously at drop, so the committed target is post-scroll.
    expect(replay).toHaveBeenCalledTimes(2)
